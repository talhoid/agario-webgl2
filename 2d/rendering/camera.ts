...
export class Camera {
	...
	followWithPadding(target: Entity, paddingPercentage: number): void {
		this.moveTo(target.position.x, target.position.y);

		const dampingFactor = 100; // Adjust this to control tapering
		const s = target.bounds.width;
		const t = target.bounds.height;

		// Calculate padding with quadratic damping
	...
	follow(target: Entity): void {
		this.followWithPadding(target, 0.2);
		// this.moveTo(target.position.x, target.position.y);
