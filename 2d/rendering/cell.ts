...
import type { Quadtree } from "@/2d/physics/quadtree";
...
export class Cell implements Drawable, Shape {
	...
	private quadtreeCache: Entity[] = [];
	private provider: GameProvider | null = null;
	private frameCount: number = 0;
	private boundsInvalidated = false;
			...
			if (this.radius > 15 && this.quadtreeCache.length > 0) {
				...
				if (!bounce) {
					for (const obj of this.quadtreeCache) {
						...
						const dx = currentX - objPos.x;
						const dy = currentY - objPos.y;
						const distanceSq = dx * dx + dy * dy;
						const radiusPlus5 = objShape.radius + 5;

						if (distanceSq > radiusPlus5 * radiusPlus5) continue;

						const objPoints = objShape.points;
						for (let k = 0; k < objPoints.length; k++) {
							const point = objPoints[k];
